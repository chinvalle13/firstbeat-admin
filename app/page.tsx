...
const totalLessons = students.reduce((sum, s) => {
const studentDays = s.lessonDays || [s.lessonDay];
const usedLessons =
countLessonDaysMultiple(s.paymentDate, studentDays) - (s.absences || 0);
return sum + Math.max(0, usedLessons);
}, 0);

const teacherPay = totalLessons * 250;
const studioNet = totalIncome - teacherPay;
...
return ( <div className="p-6 grid gap-6"> <h1 className="text-3xl font-bold">
First Beat Music Studio – Admin Portal </h1>
...
```
  <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
    ...
    <div className="p-4 border rounded-2xl shadow">
      <p className="text-sm">Teacher Pay</p>
      <p className="text-xl font-bold">₱{teacherPay}</p>
    </div>

    <div className="p-4 border rounded-2xl shadow">
      <p className="text-sm">Studio Net</p>
      <p className="text-xl font-bold">₱{studioNet}</p>
    </div>

    <div className="p-4 border rounded-2xl shadow">
      <p className="text-sm">Today's Lessons</p>
      ...
      {days.map((d) => (
        <button
          ...
          className={`px-3 py-1 border rounded ${
            lessonDays.includes(d.value) ? "bg-black text-white" : ""
          }`}
